...
class ShowSingleBook(APIView):
    def get(self, request):
        ...
        book = Book.objects.filter(id=id, is_published=True)
        data = []
        for b in book:
            data.append(
                {
...
class AddToShopcard(APIView):
    ...
    def post(self, request):
        ...
        if not Book.objects.filter(is_exist=True).exists():
            return Response({'message': 'کتاب مورد نظر موجود نمیباشد'}, status=status.HTTP_400_BAD_REQUEST)

        user = get_object_or_404(User, username=username)
        book = get_object_or_404(Book, id=id, is_published=True)
...
class ShowShopcard(APIView):
    ...
    def post(self, request):
        ...
        books = users.shopcard.filter(is_published=True)
...
class CreateFactor(APIView):
    ...
    def post(self, request):
        ...
        GateWay.objects.create(user=users, price=total_price, is_paid=False)
        return Response(status=status.HTTP_200_OK)
...
class PayFactor(APIView):
    ...
    def post(self, request):
        ...
        if factor.is_paid == True:
            return Response({"message": "شما فاکتور مورد نظر را پرداخت کرده اید."}, status=status.HTTP_400_BAD_REQUEST)

        if factor.user == users:
            factor.is_paid = True
            factor.save()
            users.shopcard.clear()
        else:
